                            ...
                            <x-dropdown align="right" width="48">
                                ...
                                <x-slot name="content">
                                    @can('rrhh.importar-usb')
                                        <x-dropdown-link :href="route('biometricos.importar')">
                                            {{ __('Importar desde USB') }}
                                        </x-dropdown-link>
                                    @endcan

                                    @can('rrhh.recuperar-datos')
                                        <x-dropdown-link :href="route('biometricos.recuperar')">
                                            {{ __('Recuperar Datos') }}
                                        </x-dropdown-link>
                                    @endcan

                                    @can('rrhh.reporte')
                                        <x-dropdown-link :href="route('biometricos.reporte')">
                                            {{ __('Generar Reporte') }}
                                        </x-dropdown-link>
                                    @endcan

                                    @can('rrhh.novedades')
                                        <x-dropdown-link :href="route('biometricos.novedades')">
                                            {{ __('Registro Novedades') }}
                                        </x-dropdown-link>
                                    @endcan
                                </x-slot>
                            </x-dropdown>
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @hasanyrole('RRHH-ADMIN|RRHH-USER|SIS-ADMIN')
                ...
                <div x-data="{ openBiometrico: false }" class="px-2">
                    <button @click="openBiometrico = !openBiometrico" class="flex items-center justify-between w-full py-2 text-sm font-medium text-gray-600 hover:text-gray-900 focus:outline-none">
                        <span>Biométricos</span>
                        <svg class="w-4 h-4 transition-transform" :class="openBiometrico ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    <div x-show="openBiometrico" class="pl-4 space-y-1">
                        @can('rrhh.importar-usb')
                            <x-responsive-nav-link :href="route('biometricos.importar')" class="block py-2 text-sm text-gray-600 hover:text-gray-900">
                                📥 Importar desde USB
                            </x-responsive-nav-link>
                        @endcan
                        @can('rrhh.recuperar-datos')
                            <x-responsive-nav-link :href="route('biometricos.recuperar')" class="block py-2 text-sm text-gray-600 hover:text-gray-900">
                                📥 Recuperar Datos
                            </x-responsive-nav-link>
                        @endcan
                        @can('rrhh.reporte')
                            <x-responsive-nav-link :href="route('biometricos.reporte')" class="block py-2 text-sm text-gray-600 hover:text-gray-900">
                                📊 Generar Reporte
                            </x-responsive-nav-link>
                        @endcan
                        @can('rrhh.novedades')
                            <x-responsive-nav-link :href="route('biometricos.novedades')" class="block py-2 text-sm text-gray-600 hover:text-gray-900">
                                📊 Registro Novedades
                            </x-responsive-nav-link>
                        @endcan
                    </div>
            ...
            @hasanyrole('SIS-ADMIN')
                ...
                <div x-data="{ openComercial: false }" class="px-2">
                    <button @click="openComercial = !openComercial" class="flex items-center justify-between w-full py-2 text-sm font-medium text-gray-600 hover:text-gray-900 focus:outline-none">
                        <span>Comercial</span>
                        <svg class="w-4 h-4 transition-transform" :class="openComercial ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    <div x-show="openComercial" class="pl-4 space-y-1">
                        @can('sis.comercial.agencias')
                            <x-responsive-nav-link :href="route('comercial.agencias.index')" class="block py-2 text-sm text-gray-600 hover:text-gray-900">
                                Agencias
                            </x-responsive-nav-link>
                        @endcan
                        @can('sis.comercial.contactos')
                            <x-responsive-nav-link :href="route('comercial.contactos.index')" class="block py-2 text-sm text-gray-600 hover:text-gray-900">
                                Contactos
                            </x-responsive-nav-link>
                        @endcan
                    </div>
