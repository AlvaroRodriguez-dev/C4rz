...
<body>
    ...
    @forelse ($audits as $audit)
        <div class="audit"><table><tr><th width="18%">Evento</th><td>{{ $audit['event'] ?? '—' }}</td><th width="18%">Usuario</th><td>{{ $audit['user_name'] ?? '—' }}</td></tr><tr><th>Auditable Type</th><td>{{ $audit['auditable_type'] ?? '—' }}</td><th>Auditable ID</th><td>{{ $audit['auditable_id'] ?? '—' }}</td></tr><tr><th>Detalle</th><td colspan="3">ID: {{ $audit['detalle_id'] ?? '—' }} · Código: {{ $audit['detalle_codigo'] ?? '—' }}</td></tr><tr><th>IP Address</th><td>{{ $audit['ip_address'] ?? '—' }}</td><th>Fecha</th><td>{{ $audit['created_at'] ?? '—' }}</td></tr><tr style="display:none"><th>Fecha</th><td colspan="3">{{ $audit['created_at'] ?? '—' }}</td></tr></table><table><tr><th width="50%">OLD VALUES</th><th width="50%">NEW VALUES</th></tr><tr><td><div class="json">{{ $audit['old_values_json'] }}</div></td><td><div class="json">{{ $audit['new_values_json'] }}</div></td></tr></table>@if(!empty($audit['cambios']))<table style="margin-top:5px"><tr><th>Campo</th><th>Antes</th><th>Después</th></tr>@foreach($audit['cambios'] as $cambio)<tr><td>{{ $cambio['campo'] }}</td><td>{{ is_scalar($cambio['antes']) || $cambio['antes'] === null ? ($cambio['antes'] ?? 'null') : $cambio['antes'] }}</td><td>{{ is_scalar($cambio['despues']) || $cambio['despues'] === null ? ($cambio['despues'] ?? 'null') : $cambio['despues'] }}</td></tr>@endforeach</table>@endif</div>
    @empty
