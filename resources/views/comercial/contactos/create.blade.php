<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Nuevo Contacto</h2>
    </x-slot>

    <div class="py-6 sm:py-8 max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
        <form action="{{ route('comercial.contactos.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="text" name="cargo" value="{{ old('cargo') }}" placeholder="Ej: EJECUTIVO DE VENTAS EMPRESARIALES"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500">
            ...
            <div>
                ...
                <select name="agencia_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500">
                    ...
                    @foreach($agencias as $agencia)
                        <option value="{{ $agencia->id }}" @selected(old('agencia_id') == $agencia->id)>
