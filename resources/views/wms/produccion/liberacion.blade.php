...
function configurarBusqueda(select){
  $(select).select2({placeholder:'Escribe código, descripción o modelo...',allowClear:true,minimumInputLength:1,width:'100%',ajax:{url:"{{ route('wms.produccion.liberacion.productos.buscar') }}",dataType:'json',delay:300,data:params=>({q:params.term||'',formato:formato.value}),processResults:data=>({results:data.results||[]}),cache:true,error:function(xhr){const mensaje=xhr.responseJSON?.message||'No fue posible consultar el catálogo de productos.';console.error('WMS RG-CB-36 - búsqueda de productos',xhr.status,xhr.responseText);mostrarAlerta(mensaje,'error');}}});
